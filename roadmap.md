...
- [x] Step 4 — questionnaire split: every question carries `part: 1 | 2` in `src/lib/prefs/questions.ts`; onboarding asks the eight essentials (home airport, purpose, cabin, seat, hotel stars, hotel rating, car setup, budget), then a bridge screen offering "Finish now" or "Keep going" for the taste questions; Preferences groups the same set under "Essentials" and "Taste"
